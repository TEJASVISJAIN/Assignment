...
const InvoiceForm = () => {
  ...
  const isEditBulk = location.pathname.includes("Bulk");

  const [editableItems, setEditableItems] = useState([]);
  const [isOpen, setIsOpen] = useState(false);
  const [copyId, setCopyId] = useState("");

  const { getOneInvoice, getAllInvoice, listSize } = useInvoiceListData();
  const [formData, setFormData] = useState(
    isEdit
      ? getOneInvoice(params.id)
      :  isEditBulk && params.id === undefined ? getAllInvoice()
      : isCopy && params.id
      ? {
          ...getOneInvoice(params.id),
          id: generateRandomId(),
  ...
  useEffect(() => { !isEditBulk && handleCalculateTotal();
  }, []);
  ...
  const handleItemChange = (index, field, value) => {    
    ...
    updatedFormData[index].items[0][field] = value;
    setFormData(updatedFormData);
  };

  const handleEditbulkInvoice = () => {
    formData.forEach((data) => {
      dispatch(updateInvoice({ id: data.id, updatedInvoice: data }));
    });
    navigate('/');
  };
  const alternatingRowClass = {
    backgroundColor: '#0000', 
  };

  const handleItemDelete = (dataIndex, itemIndex) => {
    const updatedFormData = JSON.parse(JSON.stringify(formData));
    updatedFormData[dataIndex].items.splice(itemIndex,1);
    setFormData(updatedFormData);
  };
          ...
          <Table bordered hover>
            <thead>
              <tr>
                ...
                <th className="text-center">Item Delete</th>
              </tr>
            </thead>
            <tbody>
                ...
                <React.Fragment key={`data-${index}`}>
                  {data.items.map((item, itemIndex) => (
                    <tr key={`item-${index}-${itemIndex}`} >
                      ...
                      <td>
                      <Button variant="danger" onClick={() => handleItemDelete(index, itemIndex)}>
            ...
            <Button
              variant="dark"
              className="d-block w-30 mb-2 mx-auto"
              onClick={handleEditbulkInvoice}
            >
              Save Invoice
            </Button>
      </div>
